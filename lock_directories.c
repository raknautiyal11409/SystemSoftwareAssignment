...
// void main() {
    ...
    if (pid == -1) { // fork failed
        perror("fork");
        syslog(LOG_ERR, "Locking DIR: Failed FORK ");
        exit(EXIT_FAILURE);
    }
        ...
        if (chmod(dash_dir , S_IRWXU|S_IRGRP|S_IROTH) == -1) { //change permission 
            perror("Locking DIR Error");
            syslog(LOG_ERR, "LOCK DIR: Failed to change permission %s ", shared_dir);
            exit(EXIT_FAILURE); 
        }

        printf("Permission of %s changed.\n", dash_dir);
        syslog(LOG_INFO, "LOCK DIR: Permission of %s changed.\n", dash_dir);
        exit(EXIT_SUCCESS);
    }
